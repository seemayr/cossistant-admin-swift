...
public enum DashboardIdentity {
  public struct AvatarFallbackDescriptor: Hashable, Sendable {
  ...
  public static func avatarFallback(seed: String) -> AvatarFallbackDescriptor {
    let normalizedSeed = seed.trimmingCharacters(in: .whitespacesAndNewlines)
    let resolvedSeed = normalizedSeed.isEmpty ? "visitor" : normalizedSeed
    let hash = stableHash(resolvedSeed)

    return AvatarFallbackDescriptor(
      emoji: avatarEmojis[hash % avatarEmojis.count],
      hue: Double(hash % 360) / 360,
      saturation: 0.58 + Double((hash / 360) % 16) / 100,
      brightness: 0.82 + Double((hash / 5760) % 14) / 100
    )
  }
